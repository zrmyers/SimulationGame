...
namespace Graphics {
    ...
    // Main pipeline for rendering skeletal meshes.
    class SkeletalMeshPipeline : public IPipeline {

        public:

            SkeletalMeshPipeline()
                : m_vertex_name("skeletalMesh.vert.glsl")
                , m_frag_name("skeletalMesh.frag.glsl") {
            };

            SkeletalMeshPipeline(std::string vertex_name, std::string frag_name)
            ...
            void Build(Systems::RenderSystem& rendersys, const std::string& shaderpath) override {
                ...
                vertexBufferDescription.pitch = sizeof(SkeletalMeshVertex);
                ...
                std::array<SDL_GPUVertexAttribute, 5> vertexAttributes = {};
                vertexAttributes[0].buffer_slot = 0;
                vertexAttributes[0].format = SDL_GPU_VERTEXELEMENTFORMAT_FLOAT3;
                vertexAttributes[0].location = 0;
                vertexAttributes[0].offset = offsetof(SkeletalMeshVertex, position);
                vertexAttributes[1].buffer_slot = 0;
                vertexAttributes[1].format = SDL_GPU_VERTEXELEMENTFORMAT_FLOAT3;
                vertexAttributes[1].location = 1;
                vertexAttributes[1].offset = offsetof(SkeletalMeshVertex, normal);
